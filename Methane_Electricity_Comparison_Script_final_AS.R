...
## Analyzing renewable energy (RENE) and natural gas (NG) trends for various countries ##
#' Script by Ankit Shekhar (ankit.shekhar@usys.ethz.ch)
#' Script updated date: 21st November 2022

# 0. Script Start and setup --------------------------------
...
th = theme_classic() + 
  theme(panel.background = element_rect(fill = "white", colour = "black", size = 1),
        ...
        panel.grid.minor = element_line(size = 0.1, linetype = 'dashed',colour = "grey"),
        panel.spacing = unit(0,'lines')) +
  theme(axis.text = element_text(size = 12), axis.title = element_text(size = 12+1), 
        text = element_text(size = 12), strip.text = element_text(size = 12, face = 'bold'),
        plot.title = element_text(size = 12, face = 'bold'), panel.spacing.x = unit(0.1,'lines'),strip.background = element_blank())

text_size = 12
## Theme without grid lines
th1 = theme_classic() + theme(panel.background = element_rect(fill = "white", colour = "black", size = 1),
                              panel.grid.major = element_line(size = 0, linetype = 'dashed', colour = "grey"), 
                              panel.grid.minor = element_line(size = 0, linetype = 'dashed',colour = "grey")) +
  theme(axis.text = element_text(size = text_size), axis.title = element_text(size = text_size+1, face = 'bold'), 
        text = element_text(size = text_size), strip.text = element_text(size = text_size, face = 'bold'),
        plot.title = element_text(size = text_size+1, face = 'bold'),
        panel.spacing = unit(0.01,'lines'), strip.background = element_blank(), legend.background = element_rect(fill = 'transparent'))

#1. Oktoberfest ###########
##1.1 Importing data and calculations ######

Oktoberfest_Data = read_csv("Oktoberfestdata.csv")
...
Leakage_rate_range = seq(0,0.10,0.001) ## Varying from 0 to 10%
RENE_range = seq(0,100,1)  ## Share of Rene from 0 to 100%
summary(Oktoberfest_Data)
EF_RENE_OKfest = Hydro_EF
Fossil_EF_Okfest = Oktoberfest_Data$Fossil_EF[Oktoberfest_Data$Year == 2019]
k = 1
...
for (i in c(1:length(Leakage_rate_range)))
{
  for (j in c(1:length(RENE_range))) 
  {
    ...
    Electr[k] = 0.01*RENE_range[j]*EF_RENE_OKfest + 0.01*(100-RENE_range[j])*Fossil_EF_Okfest 
    k = k+1
  }
}

RENE_leakage_DF = data.frame(NG_20 = NG20, NG_100 = NG100, LeakCH4 = Leak, RE_percent = Rene, ElectCO2 = Electr, Difference = NG20-Electr)

Desired_RENE_Okfest = min(RENE_leakage_DF$RE_percent[RENE_leakage_DF$Difference >= 0 & RENE_leakage_DF$LeakCH4 == 0])

Min_RENE_Okfest = min(RENE_leakage_DF$RE_percent[RENE_leakage_DF$Difference >= 0 & RENE_leakage_DF$LeakCH4 <= 0.0136])
...
Min_RENE_Okfest
p1 = ggplot(RENE_leakage_DF %>% filter(LeakCH4 < 0.10, RE_percent < 101), 
            aes(x = LeakCH4*100, y = RE_percent, fill = -ElectCO2+NG_20)) + 
  geom_tile() + 
  xlab('Methane leakage (%)') + 
  scale_y_continuous(name ='Renewable share (%)' , breaks = seq(0,100,20)) + 
  scale_fill_gradient2(high = 'blue',low = 'red3', n.breaks = 10, midpoint = 0,
                       name = expression('(tCO'[2]*'eq/TJ)')) +
  th + 
  guides(fill = 'none') +
  ggtitle('phase transition plot for Oktoberfest') +
  # geom_vline(xintercept = 1.36, linetype = 'dashed', color = 'red', size = 1) + 
  # geom_hline(yintercept = Desired_RENE_Okfest, linetype = 'longdash', color = 'darkgreen', size = 1) +
  # geom_hline(yintercept = Min_RENE_Okfest, linetype = 'longdash', color = 'darkorange', size = 1) +
  theme(panel.background = element_rect(fill = "white", colour = "black", size = 1), 
        legend.direction = 'vertical', legend.key.width = unit(0.3,"cm"),
        legend.background = element_rect(fill = 'transparent'),
        legend.text = element_text(size = 12, color = 'black'), legend.position = c(0.78,0.68),
        legend.text.align = 1)
p1

q1 = ggplot(Oktoberfest_Data, aes(x = Year, y = RE_Share_in_percent)) + 
  geom_point(aes(color = 'renewable share (%)')) + 
  geom_line(aes(color = 'renewable share (%)')) +
  scale_color_manual('', values = 'darkgreen') +
  scale_y_continuous(name ='Hydroelectricity share (%)' , breaks = seq(0,100,20)) + 
  ylab('Percentage share') + th + scale_x_continuous(breaks = seq(1989,2019,3)) +
  ggtitle('share of renewable energy at Oktoberfest') +  th +
  guides(color = 'none') +
  theme( legend.position = c(0.3,0.9), axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
         legend.background = element_rect(fill = 'transparent'),
         axis.title.y = element_blank(), axis.text.y = element_blank())
q1

al1 = cowplot::plot_grid(p1,q1, align = 'h', ncol = 2, rel_widths = c(2,3))
al1

# Oktoberfest plots done ---------------------------------------------------------------------------------------##

# 2. Global Analysis of 25 most NG consuming countries-----------------------------------
...
Electricity_mix_share$Country = factor(Electricity_mix_share$Country,
                                       levels = c('France','Brazil','Canada','Belgium','Venezuela','United Kingdom','Spain',
                                                  'Argentina','Italy', 'Germany','Russia','United States','Turkey','Mexico','Netherlands','Egypt','Thailand',
                                                  'Iran','Japan','Malaysia','China','Australia','India','Indonesia','Poland', 'Bangladesh','South Korea',
                                                  'Pakistan','Ukraine'))

ggplot(Electricity_mix_share %>% filter(Country %in% Countries_final, Energy_Type %in% c(Fossil,'RENE_Share')), aes(x = Year)) + 
  geom_bar(aes(y = Percent_Share*100, fill = Energy_Type), stat = 'identity', position = 'stack', width = 0.98) +
  ...
  theme(panel.background = element_rect(fill = "white", colour = "black", size = 0.3),
        panel.spacing = unit(0,'lines')) + 
  th + scale_x_continuous(limits = c(2000,2020), breaks = c(2000,2005,2010,2015,2020)) +
...
ggsave('FigureS7_Electricity_mix_with_sources.jpg', height = 8.5, width = 8, units = 'in', dpi = 800)
...
ggplot(na.omit(Renewable_Electricity) %>% filter(Country %in% Countries_final), aes(x = Year)) + 
  geom_bar(aes(y = Percent_Share*100, fill = RENE_Type), stat = 'identity', position = 'stack', alpha = 0.9, width = 0.97) +
...
ggsave('FigS8_Renewable_Electricity_mix_with_sources1.jpg', height = 8, width = 8, units = 'in', dpi = 800)
